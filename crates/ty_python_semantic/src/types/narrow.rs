...
impl<'db, 'ast> NarrowingConstraintsBuilder<'db, 'ast> {
    ...
    fn evaluate_expr_eq_or_ne(
        ...
        let narrowed = if is_positive {
            match evaluate_type_equality(self.db, lhs_ty, rhs_ty) {
                EqualityResult::EqualityIndicatesPositiveNarrowability(narrowed) => narrowed,
                EqualityResult::AlwaysEqual
                | EqualityResult::Ambiguous
                | EqualityResult::InequalityIndicatesNegativeNarrowability(_) => lhs_ty,
                EqualityResult::AlwaysUnequal => Type::Never,
            }
        } else {
            match evaluate_type_equality(self.db, lhs_ty, rhs_ty) {
                EqualityResult::InequalityIndicatesNegativeNarrowability(narrowed)
                | EqualityResult::EqualityIndicatesPositiveNarrowability(narrowed) => {
                    narrowed.negate(self.db)
                }
                EqualityResult::Ambiguous | EqualityResult::AlwaysUnequal => lhs_ty,
                EqualityResult::AlwaysEqual => Type::Never,
            }
        };
        ...
        if lhs_ty.is_single_valued(self.db) || lhs_ty.is_union_of_single_valued(self.db) {
            ...
            if let Some(lhs_union) = lhs_ty.as_union() {
                for element in lhs_union.elements(self.db) {
                    ...
                    if evaluate_type_equality(self.db, *element, rhs_values)
